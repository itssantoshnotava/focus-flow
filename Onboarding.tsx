...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleFinish = async () => {
    ...
    if (zodiac) profileUpdates.zodiacSign = zodiac;
    if (stream) profileUpdates.stream = stream;
    if (preparingForComp !== null) profileUpdates.preparingForComp = preparingForComp;
  ...
  const renderElectiveStep = () => (
    <div className="space-y-6 animate-in slide-in-from-right duration-500 text-center">
        ...
        <button 
            ...
            disabled={!elective || loading}
            className="w-full bg-white text-black hover:bg-neutral-200 disabled:opacity-50 py-3 rounded-xl font-bold mt-4 flex items-center justify-center gap-2"
        >
            {loading ? (
